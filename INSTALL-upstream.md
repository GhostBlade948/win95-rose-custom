> **Note:** this is the original Chicago95 install guide, kept for reference.
> It was written for XFCE, so the sections on the XFCE panel, Whisker Menu,
> Thunar, xfce4-terminal, the Plus! tool and Plymouth do not apply to this
> Cinnamon-only copy, and those files are not included here. The parts about
> fonts, cursors, sounds, HiDPI, Qt and GTK4 still do. For installing this
> theme, see README.md.
...
* [Additional Enhancements](#add_enhance)
    ...
    * [GTK4 and libadwaita applications](#gtk4_apps)
...
Chicago95 ships a `gtk-4.0` stylesheet alongside the GTK2 and GTK3 ones, so ordinary GTK4 applications pick the theme up from your theme setting with no extra steps.

libadwaita applications are the exception. They ignore the theme setting and always load their own stylesheet, but they do read a per-user stylesheet, so importing the theme from there reaches them as well:

    mkdir -p ~/.config/gtk-4.0
    printf '@import url("file://%s/.themes/Chicago95/gtk-4.0/gtk.css");\n' "$HOME" > ~/.config/gtk-4.0/gtk.css

Applications have to be restarted to pick this up.

*Note 1: The import names one theme explicitly. If you switch to a recoloured variant of Chicago95, change the path to match, and delete the file if you switch to an unrelated theme.*

*Note 2: libadwaita applications keep their own layout, so expect them to take on the Windows 95 palette and widgets rather than to look exactly like a 1995 application.*
